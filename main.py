...
def main():
    ...
    try:
        with closing(DataLoader()) as data_loader:
            ...
            print("阶段4: 情绪模型构建")
            print("="*50)
            sentiment_model = SentimentModel(factors_df)
            model_performance = sentiment_model.train_and_compare_models(test_size=0.3)
            print("\n模型性能比较:")
            for model, perf in model_performance.items():
                print(f"{model}: CV R²={perf['cv_score']:.4f}, Test R²={perf['test_score']:.4f}")

            print("\n因子权重分配:")
            if sentiment_model.weights:
                sorted_weights = sorted(sentiment_model.weights.items(), key=lambda item: abs(item[1]), reverse=True)
                for factor, weight in sorted_weights:
                    print(f"{factor}: {weight:.4f}")
            else:
                print("权重未计算，请检查模型训练过程。")

            sentiment_model.plot_weights()

            # 5. 创建决策引擎
            print("\n" + "="*50)
            print("阶段5: 决策引擎")
            print("="*50)
            decision_engine = TradingDecisionEngine(factors_df, sentiment_model.sentiment_scores)
            ...
            print("阶段6: 回测验证")
            print("="*50)
            backtester = StrategyBacktester(factors_df, sentiment_model.sentiment_scores)
            portfolio, benchmark = backtester.run_backtest()

            if portfolio is not None and not portfolio.empty:
                performance = backtester.calculate_performance(benchmark_returns=benchmark if not benchmark.empty else None)
                if performance:
                    print("\n策略绩效摘要:")
                    for k, v in performance.items():
                        print(f"{k}: {v:.2f}")

                backtester.plot_results(benchmark)
            else:
            ...
            try:
                ...
                if sentiment_model.sentiment_scores is not None and not sentiment_model.sentiment_scores.empty:
                    sentiment_model.sentiment_scores.to_excel(os.path.join(results_dir, '情绪得分.xlsx'))
                if portfolio is not None and not portfolio.empty:
                    portfolio.to_excel(os.path.join(results_dir, '策略净值.xlsx'))
                if 'ic_summary' in locals() and ic_summary is not None and not ic_summary.empty:
                    ic_summary.to_excel(os.path.join(results_dir, '因子IC分析.xlsx'))
                if sentiment_model.weights is not None:
                    pd.Series(sentiment_model.weights).to_excel(os.path.join(results_dir, '因子权重.xlsx'))
                if 'model_performance' in locals() and model_performance:
                    pd.DataFrame(model_performance).T.to_excel(os.path.join(results_dir, '模型性能比较.xlsx'))
        ...
        try:
            ...
            if 'sentiment_model' in locals() and hasattr(sentiment_model, 'sentiment_scores'):
                sentiment_model.sentiment_scores.to_pickle(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sentiment_scores_backup.pkl'))
            print("中间结果已备份到当前目录")
        except:
            print("无法备份中间结果")
